	// Fixed NeuroGenetics mission Beat 5 expansion —
	// expand_neurogenetics_mission_beat5 / neurogenetics_mission_beat5_v1.
	// Transforms exact Beat 4 (neurogenetics_mission_beat4_v1) → exact Beat 5.
	// Preview is mutation-free. Apply modifies only the exact DataAsset; never saves; never repairs mismatches.
	// Reuses Beat 3/4 helpers already defined above; does not modify Beat 1–4 implementations.
	const TCHAR* NeuroGeneticsMissionBeat5Spec = TEXT("neurogenetics_mission_beat5_v1");
	const TCHAR* NeuroGeneticsMissionBeat5Action = TEXT("expand_neurogenetics_mission_beat5");
	const TCHAR* NeuroGeneticsFollowEventId = TEXT("Event_NeuralSignatureFollowed");
	const TCHAR* NeuroGeneticsExamineObjectiveId = TEXT("Obj_ExamineNeuralChangeEvidence");
	const TCHAR* NeuroGeneticsExamineObjectiveTitle = TEXT("Examine the neural-change evidence");
	const TCHAR* NeuroGeneticsExamineObjectiveDescription =
		TEXT("Inspect the research-wing evidence linked to the matching neural signature.");

	TSharedRef<FJsonObject> NeuroGeneticsMissionBeat5ProposedTasks(bool bAlreadyExact)
	{
		...
		Mission->SetStringField(TEXT("shape"), TEXT("beat5"));
		...
		Mission->SetStringField(
			TEXT("runtime_task4"),
			TEXT("Locked/Inactive until Obj_FollowNeuralSignature completes, then Active via auto-unlock"));
		return Mission;
	}

	FString NeuroGeneticsMissionBeat5RejectClientOverrides(const TSharedPtr<FJsonObject>& Args)
	{
		if (!Args.IsValid())
		{
			return TEXT("");
		...
		for (const TCHAR* Key : Rejected)
		{
			if (Args->HasField(Key))
			{
				return FString::Printf(
					TEXT("expand_neurogenetics_mission_beat5 rejects client-provided '%s'. Spec neurogenetics_mission_beat5_v1 is fixed."),
					Key);
			}
		}
		const FString Spec = GetString(Args, TEXT("spec"), NeuroGeneticsMissionBeat5Spec);
		if (!Spec.Equals(NeuroGeneticsMissionBeat5Spec, ESearchCase::CaseSensitive))
		{
			return TEXT("spec must be neurogenetics_mission_beat5_v1.");
		}
		return TEXT("");
	}

	FString NeuroGeneticsMissionBeat5MismatchReason(UObject* Asset)
	{
		if (!Asset)
		{
			return TEXT("missing");
	...
	FString ApplyNeuroGeneticsMissionBeat5(UObject* Asset)
	{
		if (!Asset)
		{
	...
	FString GuardNeuroGeneticsMissionBeat5EditorContext()
	{
		if (!GIsEditor || !GEditor)
		{
			return TEXT("Editor context required. expand_neurogenetics_mission_beat5 is an Unreal Editor write.");
		}
		if (GetPieWorld())
		{
	...
	FString PreflightExpandNeuroGeneticsMissionBeat5(
		const TSharedPtr<FJsonObject>& Args,
		TSharedRef<FJsonObject> Before,
		TSharedRef<FJsonObject> Proposed)
	{
		if (const FString ContextError = GuardNeuroGeneticsMissionBeat5EditorContext(); !ContextError.IsEmpty())
		{
			return ContextError;
		}
		...
		{
			return TEXT("save/compile must be false. expand_neurogenetics_mission_beat5 does not save or compile.");
		}
		if (const FString OverrideError = NeuroGeneticsMissionBeat5RejectClientOverrides(Args); !OverrideError.IsEmpty())
		{
			return OverrideError;
		}
		if (!IsInGameThread())
		{
			return TEXT("expand_neurogenetics_mission_beat5 preflight must run on the game thread.");
		}
		...
		{
			return FString::Printf(
				TEXT("Packages dirty: %s. Require clean packages before expand_neurogenetics_mission_beat5."),
		...
		if (!Existing)
		{
			return TEXT("DA_Mission_NeuroGenetics is missing. Persist Beat 4 expand_neurogenetics_mission_beat4 + save first.");
		}
		...
		if (!bBeat4Exact && !bBeat5Exact)
		{
			return FString::Printf(
				TEXT("DA_Mission_NeuroGenetics is neither exact Beat 4 nor exact Beat 5. Beat4: %s | Beat5: %s. Fail closed — no opportunistic repair."),
				*Beat4Mismatch,
				*Beat5Mismatch);
		}

		Before->SetStringField(TEXT("spec"), NeuroGeneticsMissionBeat5Spec);
		Before->SetStringField(TEXT("action"), NeuroGeneticsMissionBeat5Action);
		...
		Before->SetObjectField(
			TEXT("mission_before"),
			bBeat5Exact ? NeuroGeneticsMissionBeat5ProposedTasks(true) : NeuroGeneticsMissionBeat4ProposedTasks(true));

		Proposed->SetStringField(TEXT("spec"), NeuroGeneticsMissionBeat5Spec);
		Proposed->SetStringField(TEXT("action"), NeuroGeneticsMissionBeat5Action);
		...
		Proposed->SetObjectField(TEXT("mission_after"), NeuroGeneticsMissionBeat5ProposedTasks(bBeat5Exact));
		Proposed->SetBoolField(TEXT("save"), false);
		Proposed->SetBoolField(TEXT("compile"), false);
		Proposed->SetBoolField(TEXT("already_exact"), bBeat5Exact);
		Proposed->SetBoolField(TEXT("idempotent"), true);
		Proposed->SetStringField(
			TEXT("result"),
			bBeat5Exact
				? TEXT("DA_Mission_NeuroGenetics already matches neurogenetics_mission_beat5_v1. Execute is a clean no-op. Requires all packages clean. No save.")
				: TEXT("Expand exact Beat 4 DA_Mission_NeuroGenetics to Beat 5: add Event_NeuralSignatureFollowed on follow task and ordered Obj_ExamineNeuralChangeEvidence. Does not save or compile."));
		return TEXT("");
	}

	TSharedRef<FJsonObject> ExecuteExpandNeuroGeneticsMissionBeat5(FBridgeChange& Change)
	{
		if (!IsInGameThread())
		{
			return FailAudit(
				TEXT("wrong_thread"),
				TEXT("expand_neurogenetics_mission_beat5 must run on the game thread. ZERO writes."),
				MakeShared<FBridgeChange>(Change));
		}

		TSharedRef<FJsonObject> Before = MakeShared<FJsonObject>();
		TSharedRef<FJsonObject> Proposed = MakeShared<FJsonObject>();
		const FString PreflightError = PreflightExpandNeuroGeneticsMissionBeat5(Change.Args, Before, Proposed);
		...
		if (NeuroGeneticsMissionBeat5MismatchReason(Existing).IsEmpty())
		{
			...
			if (DirtyAfter.Num() != 0)
			{
				...
				return FailAudit(
					TEXT("packages_dirty"),
					FString::Printf(
						TEXT("No-op exact Beat 5 found dirty packages: %s. Hard stop. Do not save."),
			...
			Change.After->SetObjectField(TEXT("mission"), NeuroGeneticsMissionBeat5ProposedTasks(true));
		...
		if (!NeuroGeneticsMissionBeat4MismatchReason(Existing).IsEmpty())
		{
			...
			return FailAudit(
				TEXT("mismatch"),
				TEXT("DA_Mission_NeuroGenetics is not exact Beat 4 at execute. Fail closed. ZERO writes."),
				MakeShared<FBridgeChange>(Change));
		}
		...
		UObject* Snapshot = StaticDuplicateObject(
			Existing,
			GetTransientPackage(),
			*FString::Printf(TEXT("%s_Beat4Snapshot"), *Existing->GetName()));
		if (!Snapshot)
		{
			Change.Status = TEXT("execute_failed");
			return FailAudit(
				TEXT("snapshot_failed"),
				TEXT("Failed to snapshot Beat 4 mission for rollback. ZERO writes."),
				MakeShared<FBridgeChange>(Change));
		}

		{
			const FScopedTransaction Transaction(NSLOCTEXT(
				"OrganoidAIBridge",
				"ExpandNeuroGeneticsMissionBeat5",
				"Expand NeuroGenetics Mission Beat 5"));
			if (const FString ApplyError = ApplyNeuroGeneticsMissionBeat5(Existing); !ApplyError.IsEmpty())
			{
				...
				return FailAudit(
					TEXT("apply_failed"),
					FString::Printf(
						TEXT("%s Rolled back to Beat 4 snapshot. %s ZERO remaining writes."),
						*ApplyError,
		...
		if (const FString VerifyError = NeuroGeneticsMissionBeat5MismatchReason(Existing); !VerifyError.IsEmpty())
		{
			TArray<FString> Restored;
			const FString RestoreError =
				RestoreNeuroGeneticsMissionFromSnapshot(Existing, Snapshot, bPackageWasDirtyBefore, Restored);
		...
		Change.After->SetObjectField(TEXT("mission_before"), NeuroGeneticsMissionBeat4ProposedTasks(true));
		Change.After->SetObjectField(TEXT("mission_after"), NeuroGeneticsMissionBeat5ProposedTasks(false));
		Change.After->SetArrayField(TEXT("dirty_packages"), DirtyPackageJsonArray(DirtyAfter));
		LogAudit(TEXT("execute"), Change);
		return Ok(AuditBase(Change));
	}
