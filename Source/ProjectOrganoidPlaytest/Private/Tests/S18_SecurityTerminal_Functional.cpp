...
namespace
{
	constexpr TCHAR TestId[] = TEXT("S18_SecurityTerminal_Functional");
	constexpr TCHAR DisplayName[] = TEXT("Section 18 Security Terminal Functional");
	...
	constexpr TCHAR TerminalLabel[] = TEXT("Admin_Terminal_Security");
	constexpr TCHAR ReceptionLabel[] = TEXT("Admin_Terminal_Reception");
	constexpr TCHAR ExpectedTerminalId[] = TEXT("Terminal_AdminSecurity");
	constexpr TCHAR ExpectedTitle[] = TEXT("Security Terminal");
	...
	const FVector ExpectedLocation(2680.0f, -400.0f, 110.0f);
	...
	bool IsSecurityEnum(const FOrganoidPlaytestPropValue& Value)
	{
		if (Value.EnumDisplay.Equals(TEXT("Security"), ESearchCase::IgnoreCase))
		{
			return true;
		}
		if (Value.EnumInternal.Equals(TEXT("NewEnumerator1"), ESearchCase::IgnoreCase))
		{
			return true;
		}
		if (Value.Text.Equals(TEXT("Security"), ESearchCase::IgnoreCase))
		{
			return true;
		}
		return Value.bHasNumber && FMath::IsNearlyEqual(Value.Number, 1.0);
	}
	...
	class FS18SecurityTerminalFunctional : public IOrganoidPlaytestCase
	{
	public:
	...
	private:
		...
		void TickWaitPieReady(UProjectOrganoidPlaytestEditorSubsystem& Owner, FOrganoidPlaytestRecord& Record, float DeltaTime)
		{
			...
			if (WaitSeconds > 45.0f)
			{
				...
				Owner.CompleteActive(
					...
					FString::Printf(
						TEXT("Timed out waiting for PIE player + Admin_Terminal_Security (world=%s pawn=%s terminals=%d walking=%s)."),
		...
		FReceptionSnapshot CaptureReception(UWorld* World) const
		{
			FReceptionSnapshot Snap;
			TArray<AActor*> Receptions = OrganoidPlaytestActions::FindActorsByLabel(World, ReceptionLabel);
			Snap.Count = Receptions.Num();
			if (Receptions.Num() == 1)
			{
				AActor* Actor = Receptions[0];
		...
		void TickAssertConfig(UProjectOrganoidPlaytestEditorSubsystem& Owner, FOrganoidPlaytestRecord& Record)
		{
			...
			AssertTrue(
				Record,
				TEXT("exactly_one_security_terminal"),
				Terminals.Num() == 1,
				TEXT("1"),
			...
			if (Terminals.Num() != 1)
			{
				Record.MarkNeedsApproval(
					TEXT("Security terminal count is not exactly one."),
			...
			AssertTrue(
				...
				TerminalType.bFound && IsSecurityEnum(TerminalType),
				TEXT("Security / NewEnumerator1 / 1"),
			...
			ReceptionBefore = CaptureReception(World);
			TArray<AActor*> Receptions = OrganoidPlaytestActions::FindActorsByLabel(World, ReceptionLabel);
			AssertTrue(
				Record, TEXT("reception_unique"),
				ReceptionBefore.Count == 1,
				TEXT("1"),
				FString::FromInt(ReceptionBefore.Count),
				ReceptionLabel, true);
			if (Receptions.Num() == 1)
			{
				Reception = Receptions[0];
				Record.AddActor(TEXT("reception"), OrganoidPlaytestActions::ActorLabel(Receptions[0]));
				const bool bReceptionLoc =
				...
				AssertTrue(
					...
					bReceptionLoc
						...
						&& ReceptionBefore.Title.Equals(TEXT("Reception Terminal")),
			...
			{
				Record.MarkNeedsApproval(
					TEXT("Durable Security configuration does not match the expected Section 18 instance."),
					TEXT("Use OrganoidAIBridge prepare_write / dual approve / execute_write. The playtest bot will not mutate."));
				FailAndStop(Owner, Record, Record.FailureReason);
				return;
			}
		...
		void AssertReceptionUnchanged(FOrganoidPlaytestRecord& Record, const FString& Prefix)
		{
			UWorld* World = OrganoidPlaytestActions::GetPieWorld();
			const FReceptionSnapshot Now = CaptureReception(World);
			const bool bSame =
				Now.Count == ReceptionBefore.Count
				&& Now.Count == 1
				&& Now.Location.Equals(ReceptionBefore.Location, 1.0f)
				&& Now.Rotation.Equals(ReceptionBefore.Rotation, 0.5f)
				&& Now.Scale.Equals(ReceptionBefore.Scale, 0.01f)
				&& Now.TerminalID == ReceptionBefore.TerminalID
				&& Now.Title == ReceptionBefore.Title
				&& Now.TerminalType == ReceptionBefore.TerminalType
				&& Now.bInitiallyPowered == ReceptionBefore.bInitiallyPowered
				&& Now.bOneShot == ReceptionBefore.bOneShot
				&& Now.bHasActivated == ReceptionBefore.bHasActivated
				&& Now.bIsInteractable == ReceptionBefore.bIsInteractable;
			AssertTrue(
				Record, FString::Printf(TEXT("%s.reception_unchanged"), *Prefix),
				bSame,
				TEXT("unique Reception transform+config unchanged"),
				FString::Printf(TEXT("count=%d id=%s activated=%s"), Now.Count, *Now.TerminalID, *BoolText(Now.bHasActivated)),
				ReceptionLabel,
				false);
		}
		...
		void TickAssertAfterInteract(UProjectOrganoidPlaytestEditorSubsystem& Owner, FOrganoidPlaytestRecord& Record, float DeltaTime)
		{
			...
			if (InteractAttempt == 1)
			{
				AssertTrue(Record, TEXT("powered_path"), bScreen && !bUnpowered, TEXT("AdminTerminal Screen, no unpowered error"),
					...
					TerminalLabel, false);
				AssertTrue(Record, TEXT("security_route"), bSecurity, TEXT("AdminTerminal OnSecurityActivated"),
					bSecurity ? TEXT("AdminTerminal OnSecurityActivated") : TEXT("<missing>"),
					TerminalLabel, false);
			...
			{
				AssertTrue(Record, TEXT("repeat_powered_or_security"), bScreen || bSecurity, TEXT("powered/security print on second use"),
					FString::Printf(TEXT("screen=%s security=%s"), *BoolText(bScreen), *BoolText(bSecurity)),
					TerminalLabel, false);
			}
			...
			AssertReceptionUnchanged(Record, Prefix);
	...
	struct FS18SecurityAutoRegister
	{
		FS18SecurityAutoRegister()
		{
			...
			{
				return MakeShared<FS18SecurityTerminalFunctional>();
			};
			FOrganoidPlaytestRegistry::Register(Entry);
		}
	};

	static FS18SecurityAutoRegister GRegisterS18Security;
}
