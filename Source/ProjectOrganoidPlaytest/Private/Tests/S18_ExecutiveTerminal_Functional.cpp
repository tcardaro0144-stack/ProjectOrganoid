...
namespace
{
	constexpr TCHAR TestId[] = TEXT("S18_ExecutiveTerminal_Functional");
	constexpr TCHAR DisplayName[] = TEXT("Section 18 Executive Terminal Functional");
	...
	constexpr TCHAR TerminalLabel[] = TEXT("Admin_Terminal_Executive");
	...
	constexpr TCHAR ExpectedTerminalId[] = TEXT("Terminal_AdminExecutive");
	constexpr TCHAR ExpectedTitle[] = TEXT("Executive Terminal");
	...
	const FVector ExpectedLocation(1840.0f, -1850.0f, 110.0f);
	const FRotator ExpectedRotation(0.0f, 0.0f, 0.0f);
	const FVector ExpectedScale(1.0f, 1.0f, 1.0f);
	const FVector VisitorEastDirection(1.0f, 0.0f, 0.0f);
	...
	bool IsExecutiveEnum(const FOrganoidPlaytestPropValue& Value)
	{
		if (Value.EnumDisplay.Equals(TEXT("Executive"), ESearchCase::IgnoreCase))
		{
			return true;
		}
		if (Value.EnumInternal.Equals(TEXT("NewEnumerator3"), ESearchCase::IgnoreCase))
		{
			return true;
		}
		if (Value.Text.Equals(TEXT("Executive"), ESearchCase::IgnoreCase))
		{
			return true;
		}
		return Value.bHasNumber && FMath::IsNearlyEqual(Value.Number, 3.0);
	}
	...
	class FS18ExecutiveTerminalFunctional : public IOrganoidPlaytestCase
	{
	public:
	...
	private:
		...
		void TickWaitPieReady(UProjectOrganoidPlaytestEditorSubsystem& Owner, FOrganoidPlaytestRecord& Record, float DeltaTime)
		{
			...
			if (WaitSeconds > 45.0f)
			{
				...
				Owner.CompleteActive(
					...
					FString::Printf(
						TEXT("Timed out waiting for PIE player + Admin_Terminal_Executive (world=%s pawn=%s terminals=%d walking=%s)."),
		...
		void TickAssertConfig(UProjectOrganoidPlaytestEditorSubsystem& Owner, FOrganoidPlaytestRecord& Record)
		{
			...
			AssertTrue(
				Record,
				TEXT("exactly_one_executive_terminal"),
				Terminals.Num() == 1,
				TEXT("1"),
			...
			if (Terminals.Num() != 1)
			{
				Record.MarkNeedsApproval(
					TEXT("Executive terminal count is not exactly one."),
			...
			AssertTrue(
				...
				TerminalType.bFound && IsExecutiveEnum(TerminalType),
				TEXT("Executive / NewEnumerator3 / 3"),
			...
			{
				Record.MarkNeedsApproval(
					TEXT("Durable Executive configuration does not match the expected Section 18 instance."),
					TEXT("Use OrganoidAIBridge prepare_write / dual approve / execute_write. The playtest bot will not mutate."));
				FailAndStop(Owner, Record, Record.FailureReason);
				return;
			}
		...
		void TickTeleportFace(UProjectOrganoidPlaytestEditorSubsystem& Owner, FOrganoidPlaytestRecord& Record)
		{
			...
			OrganoidPlaytestActions::TeleportNear(Pawn, Target->GetActorLocation(), SetupDistance, CapsuleZ, VisitorEastDirection);
		...
		void TickWaitFocus(UProjectOrganoidPlaytestEditorSubsystem& Owner, FOrganoidPlaytestRecord& Record, float DeltaTime)
		{
			...
			const bool bVisitorEast = Pawn->GetActorLocation().X > Target->GetActorLocation().X;

			if (bRange && bFacing && bFocused && bVisitorEast)
			{
				AssertTrue(Record, TEXT("player_in_range"), true, TEXT("75-150"), FString::SanitizeFloat(Dist), TerminalLabel, false);
				AssertTrue(Record, TEXT("player_visitor_east_side"), true, TEXT("player.X > terminal.X ~ (1940,-1850)"), FString::SanitizeFloat(Pawn->GetActorLocation().X), TerminalLabel, false);
			...
			if (WaitSeconds > 3.0f)
			{
				...
				AssertTrue(Record, TEXT("player_visitor_east_side"), bVisitorEast, TEXT("player.X > terminal.X ~ (1940,-1850)"), FString::SanitizeFloat(Pawn->GetActorLocation().X), TerminalLabel, false);
				...
				AssertTrue(
					...
					TerminalLabel, false);
				FailAndStop(Owner, Record, TEXT("Failed to acquire genuine interaction focus after visitor-east teleport/face."));
			}
		}
		...
		void TickAssertAfterInteract(UProjectOrganoidPlaytestEditorSubsystem& Owner, FOrganoidPlaytestRecord& Record, float DeltaTime)
		{
			...
			if (InteractAttempt == 1)
			{
				AssertTrue(Record, TEXT("powered_path"), bScreen && !bUnpowered, TEXT("AdminTerminal Screen, no unpowered error"),
					...
					TerminalLabel, false);
				AssertTrue(Record, TEXT("executive_route"), bExecutive, TEXT("AdminTerminal OnExecutiveActivated"),
					bExecutive ? TEXT("AdminTerminal OnExecutiveActivated") : TEXT("<missing>"),
					TerminalLabel, false);
			...
			{
				AssertTrue(Record, TEXT("repeat_powered_or_executive"), bScreen || bExecutive, TEXT("powered/executive print on second use"),
					FString::Printf(TEXT("screen=%s executive=%s"), *BoolText(bScreen), *BoolText(bExecutive)),
					TerminalLabel, false);
			}
	...
	struct FS18ExecutiveAutoRegister
	{
		FS18ExecutiveAutoRegister()
		{
			...
			{
				return MakeShared<FS18ExecutiveTerminalFunctional>();
			};
			FOrganoidPlaytestRegistry::Register(Entry);
		}
	};

	static FS18ExecutiveAutoRegister GRegisterS18Executive;
}
