...
namespace
{
	constexpr TCHAR TestId[] = TEXT("S18_RecordsTerminal_Functional");
	constexpr TCHAR DisplayName[] = TEXT("Section 18 Records Terminal Functional");
	...
	constexpr TCHAR TerminalLabel[] = TEXT("Admin_Terminal_Records");
	constexpr TCHAR ReceptionLabel[] = TEXT("Admin_Terminal_Reception");
	constexpr TCHAR SecurityLabel[] = TEXT("Admin_Terminal_Security");
	constexpr TCHAR ExpectedTerminalId[] = TEXT("Terminal_AdminRecords");
	constexpr TCHAR ExpectedTitle[] = TEXT("Records Terminal");
	...
	const FVector ExpectedLocation(2680.0f, 332.0f, 110.0f);
	const FRotator ExpectedRotation(0.0f, 0.0f, 0.0f);
	const FVector ExpectedScale(1.0f, 1.0f, 1.0f);
	const FVector SouthHubDirection(0.0f, -1.0f, 0.0f);
	...
	bool IsRecordsEnum(const FOrganoidPlaytestPropValue& Value)
	{
		if (Value.EnumDisplay.Equals(TEXT("Records"), ESearchCase::IgnoreCase))
		{
			return true;
		}
		if (Value.EnumInternal.Equals(TEXT("NewEnumerator2"), ESearchCase::IgnoreCase))
		{
			return true;
		}
		if (Value.Text.Equals(TEXT("Records"), ESearchCase::IgnoreCase))
		{
			return true;
		}
		return Value.bHasNumber && FMath::IsNearlyEqual(Value.Number, 2.0);
	}
	...
	class FS18RecordsTerminalFunctional : public IOrganoidPlaytestCase
	{
	public:
	...
	private:
		...
		void TickWaitPieReady(UProjectOrganoidPlaytestEditorSubsystem& Owner, FOrganoidPlaytestRecord& Record, float DeltaTime)
		{
			...
			if (WaitSeconds > 45.0f)
			{
				...
				Owner.CompleteActive(
					...
					FString::Printf(
						TEXT("Timed out waiting for PIE player + Admin_Terminal_Records (world=%s pawn=%s terminals=%d walking=%s)."),
		...
		void TickAssertConfig(UProjectOrganoidPlaytestEditorSubsystem& Owner, FOrganoidPlaytestRecord& Record)
		{
			...
			AssertTrue(
				Record,
				TEXT("exactly_one_records_terminal"),
				Terminals.Num() == 1,
				TEXT("1"),
			...
			if (Terminals.Num() != 1)
			{
				Record.MarkNeedsApproval(
					TEXT("Records terminal count is not exactly one."),
			...
			AssertTrue(
				...
				TerminalType.bFound && IsRecordsEnum(TerminalType),
				TEXT("Records / NewEnumerator2 / 2"),
			...
			{
				Record.MarkNeedsApproval(
					TEXT("Durable Records configuration does not match the expected Section 18 instance."),
					TEXT("Use OrganoidAIBridge prepare_write / dual approve / execute_write. The playtest bot will not mutate."));
				FailAndStop(Owner, Record, Record.FailureReason);
				return;
			}
		...
		void TickTeleportFace(UProjectOrganoidPlaytestEditorSubsystem& Owner, FOrganoidPlaytestRecord& Record)
		{
			...
			OrganoidPlaytestActions::TeleportNear(Pawn, Target->GetActorLocation(), SetupDistance, CapsuleZ, SouthHubDirection);
		...
		void TickWaitFocus(UProjectOrganoidPlaytestEditorSubsystem& Owner, FOrganoidPlaytestRecord& Record, float DeltaTime)
		{
			...
			const bool bSouth = Pawn->GetActorLocation().Y < Target->GetActorLocation().Y;

			if (bRange && bFacing && bFocused && bSouth)
			{
				AssertTrue(Record, TEXT("player_in_range"), true, TEXT("75-150"), FString::SanitizeFloat(Dist), TerminalLabel, false);
				AssertTrue(Record, TEXT("player_south_hub_side"), true, TEXT("player.Y < terminal.Y"), FString::SanitizeFloat(Pawn->GetActorLocation().Y), TerminalLabel, false);
			...
			if (WaitSeconds > 3.0f)
			{
				...
				AssertTrue(Record, TEXT("player_south_hub_side"), bSouth, TEXT("player.Y < terminal.Y"), FString::SanitizeFloat(Pawn->GetActorLocation().Y), TerminalLabel, false);
				...
				AssertTrue(
					...
					TerminalLabel, false);
				FailAndStop(Owner, Record, TEXT("Failed to acquire genuine interaction focus after south/hub teleport/face."));
			}
		}
		...
		void TickAssertAfterInteract(UProjectOrganoidPlaytestEditorSubsystem& Owner, FOrganoidPlaytestRecord& Record, float DeltaTime)
		{
			...
			if (InteractAttempt == 1)
			{
				AssertTrue(Record, TEXT("powered_path"), bScreen && !bUnpowered, TEXT("AdminTerminal Screen, no unpowered error"),
					...
					TerminalLabel, false);
				AssertTrue(Record, TEXT("records_route"), bRecords, TEXT("AdminTerminal OnRecordsActivated"),
					bRecords ? TEXT("AdminTerminal OnRecordsActivated") : TEXT("<missing>"),
					TerminalLabel, false);
			...
			{
				AssertTrue(Record, TEXT("repeat_powered_or_records"), bScreen || bRecords, TEXT("powered/records print on second use"),
					FString::Printf(TEXT("screen=%s records=%s"), *BoolText(bScreen), *BoolText(bRecords)),
					TerminalLabel, false);
			}
	...
	struct FS18RecordsAutoRegister
	{
		FS18RecordsAutoRegister()
		{
			...
			{
				return MakeShared<FS18RecordsTerminalFunctional>();
			};
			FOrganoidPlaytestRegistry::Register(Entry);
		}
	};

	static FS18RecordsAutoRegister GRegisterS18Records;
}
