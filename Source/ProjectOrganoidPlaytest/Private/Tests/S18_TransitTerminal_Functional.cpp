...
namespace
{
	constexpr TCHAR TestId[] = TEXT("S18_TransitTerminal_Functional");
	constexpr TCHAR DisplayName[] = TEXT("Section 18 Transit Terminal Functional");
	...
	constexpr TCHAR TerminalLabel[] = TEXT("Admin_Terminal_Transit");
	constexpr TCHAR ReceptionLabel[] = TEXT("Admin_Terminal_Reception");
	constexpr TCHAR SecurityLabel[] = TEXT("Admin_Terminal_Security");
	constexpr TCHAR RecordsLabel[] = TEXT("Admin_Terminal_Records");
	constexpr TCHAR ExecutiveLabel[] = TEXT("Admin_Terminal_Executive");
	constexpr TCHAR OperationsLabel[] = TEXT("Admin_Terminal_Operations");
	constexpr TCHAR ExpectedTerminalId[] = TEXT("Terminal_AdminTransit");
	constexpr TCHAR ExpectedTitle[] = TEXT("Transit Terminal");
	...
	const FVector ExpectedLocation(5380.0f, 0.0f, 110.0f);
	const FRotator ExpectedRotation(0.0f, 0.0f, 0.0f);
	...
	bool IsTransitEnum(const FOrganoidPlaytestPropValue& Value)
	{
		if (Value.EnumDisplay.Equals(TEXT("Transit"), ESearchCase::IgnoreCase))
		{
			return true;
		}
		if (Value.EnumInternal.Equals(TEXT("NewEnumerator5"), ESearchCase::IgnoreCase))
		{
			return true;
		}
		if (Value.Text.Equals(TEXT("Transit"), ESearchCase::IgnoreCase))
		{
			return true;
		}
		return Value.bHasNumber && FMath::IsNearlyEqual(Value.Number, 5.0);
	}
	...
	FString TransformText(const FVector& Location, const FRotator& Rotation, const FVector& Scale)
	{
	...
	class FS18TransitTerminalFunctional : public IOrganoidPlaytestCase
	{
	public:
	...
	private:
		...
		struct FNeighborSnapshot
		{
			...
			bool bInitiallyPowered = false;
			bool bOneShot = true;
			bool bHasActivated = true;
		...
		FNeighborSnapshot ReceptionBefore;
		FNeighborSnapshot SecurityBefore;
		FNeighborSnapshot RecordsBefore;
		FNeighborSnapshot ExecutiveBefore;
		FNeighborSnapshot OperationsBefore;
		...
		void TickWaitPieReady(UProjectOrganoidPlaytestEditorSubsystem& Owner, FOrganoidPlaytestRecord& Record, float DeltaTime)
		{
			...
			if (WaitSeconds > 45.0f)
			{
				...
				Owner.CompleteActive(
					...
					FString::Printf(
						TEXT("Timed out waiting for PIE player + Admin_Terminal_Transit (world=%s pawn=%s terminals=%d walking=%s)."),
		...
		FNeighborSnapshot CaptureNeighbor(UWorld* World, const TCHAR* Label) const
		{
			FNeighborSnapshot Snap;
			TArray<AActor*> Found = OrganoidPlaytestActions::FindActorsByLabel(World, Label);
			Snap.Count = Found.Num();
			if (Found.Num() == 1)
			{
				AActor* Actor = Found[0];
				Snap.Location = Actor->GetActorLocation();
				Snap.Rotation = Actor->GetActorRotation();
				Snap.Scale = Actor->GetActorScale3D();
				Snap.TerminalID = OrganoidPlaytestActions::ReadProperty(Actor, TEXT("TerminalID")).Text;
				Snap.Title = OrganoidPlaytestActions::ReadProperty(Actor, TEXT("Title")).Text;
				const FOrganoidPlaytestPropValue Type = OrganoidPlaytestActions::ReadProperty(Actor, TEXT("TerminalType"));
				Snap.TerminalType = FString::Printf(TEXT("%s/%s/%.0f"), *Type.EnumDisplay, *Type.EnumInternal, Type.Number);
				const FOrganoidPlaytestPropValue Init = OrganoidPlaytestActions::ReadProperty(Actor, TEXT("bInitiallyPowered"));
				Snap.bInitiallyPowered = Init.bHasBool && Init.bBool;
				const FOrganoidPlaytestPropValue OneShot = OrganoidPlaytestActions::ReadProperty(Actor, TEXT("bOneShot"));
				Snap.bOneShot = OneShot.bHasBool && OneShot.bBool;
				const FOrganoidPlaytestPropValue HasActivated = OrganoidPlaytestActions::ReadProperty(Actor, TEXT("bHasActivated"));
				Snap.bHasActivated = HasActivated.bHasBool && HasActivated.bBool;
				const FOrganoidPlaytestPropValue Interactable = OrganoidPlaytestActions::ReadProperty(Actor, TEXT("bIsInteractable"));
				Snap.bIsInteractable = Interactable.bHasBool && Interactable.bBool;
			}
			return Snap;
		}
		...
		void TickAssertConfig(UProjectOrganoidPlaytestEditorSubsystem& Owner, FOrganoidPlaytestRecord& Record)
		{
			...
			AssertTrue(
				Record,
				TEXT("exactly_one_transit_terminal"),
				Terminals.Num() == 1,
				TEXT("1"),
			...
			if (Terminals.Num() != 1)
			{
				Record.MarkNeedsApproval(
					TEXT("Transit terminal count is not exactly one."),
			...
			const bool bTransformOk =
				Location.Equals(ExpectedLocation, 1.0f)
				&& Rotation.Equals(ExpectedRotation, 0.5f)
				&& Scale.Equals(ExpectedScale, 0.01f);
			AssertTrue(
				Record, TEXT("transform"),
				bTransformOk,
				TransformText(ExpectedLocation, ExpectedRotation, ExpectedScale),
				TransformText(Location, Rotation, Scale),
				TerminalLabel, true);
			...
			AssertTrue(
				...
				TerminalType.bFound && IsTransitEnum(TerminalType),
				TEXT("Transit / NewEnumerator5 / 5"),
			...
			ReceptionBefore = CaptureNeighbor(World, ReceptionLabel);
			SecurityBefore = CaptureNeighbor(World, SecurityLabel);
			RecordsBefore = CaptureNeighbor(World, RecordsLabel);
			ExecutiveBefore = CaptureNeighbor(World, ExecutiveLabel);
			OperationsBefore = CaptureNeighbor(World, OperationsLabel);
			AssertTrue(Record, TEXT("reception_unique"), ReceptionBefore.Count == 1, TEXT("1"), FString::FromInt(ReceptionBefore.Count), ReceptionLabel, true);
			AssertTrue(
				Record, TEXT("reception_unchanged_initial"),
				ReceptionBefore.Count == 1
					&& ReceptionBefore.Location.Equals(FVector(1450.0f, -150.0f, 110.0f), 1.0f)
					&& ReceptionBefore.TerminalID.Equals(TEXT("Terminal_AdminReception")),
				TEXT("unique Reception at (1450,-150,110)"),
				TransformText(ReceptionBefore.Location, ReceptionBefore.Rotation, ReceptionBefore.Scale),
				ReceptionLabel, true);
			AssertTrue(Record, TEXT("security_unique"), SecurityBefore.Count == 1, TEXT("1"), FString::FromInt(SecurityBefore.Count), SecurityLabel, true);
			AssertTrue(
				Record, TEXT("security_unchanged_initial"),
				SecurityBefore.Count == 1
					&& SecurityBefore.Location.Equals(FVector(2680.0f, -400.0f, 110.0f), 1.0f)
					&& SecurityBefore.TerminalID.Equals(TEXT("Terminal_AdminSecurity")),
				TEXT("unique Security at (2680,-400,110)"),
				TransformText(SecurityBefore.Location, SecurityBefore.Rotation, SecurityBefore.Scale),
				SecurityLabel, true);
			AssertTrue(Record, TEXT("records_unique"), RecordsBefore.Count == 1, TEXT("1"), FString::FromInt(RecordsBefore.Count), RecordsLabel, true);
			AssertTrue(
				Record, TEXT("records_unchanged_initial"),
				RecordsBefore.Count == 1
					&& RecordsBefore.Location.Equals(FVector(2680.0f, 332.0f, 110.0f), 1.0f)
					&& RecordsBefore.TerminalID.Equals(TEXT("Terminal_AdminRecords")),
				TEXT("unique Records at (2680,332,110)"),
				TransformText(RecordsBefore.Location, RecordsBefore.Rotation, RecordsBefore.Scale),
				RecordsLabel, true);
			AssertTrue(Record, TEXT("executive_unique"), ExecutiveBefore.Count == 1, TEXT("1"), FString::FromInt(ExecutiveBefore.Count), ExecutiveLabel, true);
			AssertTrue(
				Record, TEXT("executive_unchanged_initial"),
				ExecutiveBefore.Count == 1
					&& ExecutiveBefore.Location.Equals(FVector(1840.0f, -1850.0f, 110.0f), 1.0f)
					&& ExecutiveBefore.TerminalID.Equals(TEXT("Terminal_AdminExecutive")),
				TEXT("unique Executive at (1840,-1850,110)"),
				TransformText(ExecutiveBefore.Location, ExecutiveBefore.Rotation, ExecutiveBefore.Scale),
				ExecutiveLabel, true);
			AssertTrue(Record, TEXT("operations_unique"), OperationsBefore.Count == 1, TEXT("1"), FString::FromInt(OperationsBefore.Count), OperationsLabel, true);
			AssertTrue(
				Record, TEXT("operations_unchanged_initial"),
				OperationsBefore.Count == 1
					&& OperationsBefore.Location.Equals(FVector(4435.0f, 280.0f, 110.0f), 1.0f)
					&& OperationsBefore.TerminalID.Equals(TEXT("Terminal_AdminOperations")),
				TEXT("unique Operations at (4435,280,110)"),
				TransformText(OperationsBefore.Location, OperationsBefore.Rotation, OperationsBefore.Scale),
				OperationsLabel, true);
			if (ReceptionBefore.Count == 1)
			{
				Record.AddActor(TEXT("reception"), ReceptionLabel);
			}
			if (SecurityBefore.Count == 1)
			{
				Record.AddActor(TEXT("security"), SecurityLabel);
			}
			if (RecordsBefore.Count == 1)
			{
				Record.AddActor(TEXT("records"), RecordsLabel);
			}
			if (ExecutiveBefore.Count == 1)
			{
				Record.AddActor(TEXT("executive"), ExecutiveLabel);
			}
			if (OperationsBefore.Count == 1)
			{
				Record.AddActor(TEXT("operations"), OperationsLabel);
			}
			...
			if (AActor* FoundDoor = OrganoidPlaytestActions::FindAccessDoor(World))
			{
				...
				AssertTrue(
					Record, TEXT("access_door_initial"),
					DoorLocation.Equals(FVector(800.0f, 0.0f, 0.0f), 1.0f),
					TEXT("(800,0,0)"),
			...
			{
				Record.MarkNeedsApproval(
					TEXT("Durable Transit configuration does not match the expected Section 18 instance."),
					TEXT("Use OrganoidAIBridge prepare_write / dual approve / execute_write. The playtest bot will not mutate."));
				FailAndStop(Owner, Record, Record.FailureReason);
				return;
			}
		...
		void TickWaitFocus(UProjectOrganoidPlaytestEditorSubsystem& Owner, FOrganoidPlaytestRecord& Record, float DeltaTime)
		{
			...
			const bool bVisitorWest = Pawn->GetActorLocation().X < Target->GetActorLocation().X;

			if (bRange && bFacing && bFocused && bVisitorWest)
			{
				AssertTrue(Record, TEXT("player_in_range"), true, TEXT("75-150"), FString::SanitizeFloat(Dist), TerminalLabel, false);
				AssertTrue(Record, TEXT("player_visitor_west_side"), true, TEXT("player.X < terminal.X ~ (5280,0)"), FString::SanitizeFloat(Pawn->GetActorLocation().X), TerminalLabel, false);
			...
			if (WaitSeconds > 3.0f)
			{
				...
				AssertTrue(Record, TEXT("player_visitor_west_side"), bVisitorWest, TEXT("player.X < terminal.X ~ (5280,0)"), FString::SanitizeFloat(Pawn->GetActorLocation().X), TerminalLabel, false);
				...
				AssertTrue(
					...
					TerminalLabel, false);
				FailAndStop(Owner, Record, TEXT("Failed to acquire genuine interaction focus after visitor-west teleport/face."));
			}
		}
		...
		bool NeighborUnchanged(const FNeighborSnapshot& Before, const FNeighborSnapshot& Now) const
		{
			return Now.Count == Before.Count
				&& Now.Count == 1
				&& Now.Location.Equals(Before.Location, 1.0f)
				&& Now.Rotation.Equals(Before.Rotation, 0.5f)
				&& Now.Scale.Equals(Before.Scale, 0.01f)
				&& Now.TerminalID == Before.TerminalID
				&& Now.Title == Before.Title
				&& Now.TerminalType == Before.TerminalType
				&& Now.bInitiallyPowered == Before.bInitiallyPowered
				&& Now.bOneShot == Before.bOneShot
				&& Now.bHasActivated == Before.bHasActivated
				&& Now.bIsInteractable == Before.bIsInteractable;
		}
		...
		void TickAssertAfterInteract(UProjectOrganoidPlaytestEditorSubsystem& Owner, FOrganoidPlaytestRecord& Record, float DeltaTime)
		{
			...
			const bool bExecutive = Sink && Sink->ContainsSinceCursor(TEXT("AdminTerminal OnExecutiveActivated"));
			const bool bRecords = Sink && Sink->ContainsSinceCursor(TEXT("AdminTerminal OnRecordsActivated"));
			...
			const bool bOperations = Sink && Sink->ContainsSinceCursor(TEXT("AdminTerminal OnOperationsActivated"));
			const bool bTransit = Sink && Sink->ContainsSinceCursor(TEXT("AdminTerminal OnTransitActivated"));
			...
			if (InteractAttempt == 1)
			{
				AssertTrue(Record, TEXT("powered_path"), bScreen && !bUnpowered, TEXT("AdminTerminal Screen, no unpowered error"),
					...
					TerminalLabel, false);
				AssertTrue(Record, TEXT("transit_route"), bTransit, TEXT("AdminTerminal OnTransitActivated"),
					bTransit ? TEXT("AdminTerminal OnTransitActivated") : TEXT("<missing>"),
					TerminalLabel, false);
				AssertTrue(Record, TEXT("no_reception_route"), !bReception, TEXT("OnReceptionActivated absent"),
					bReception ? TEXT("AdminTerminal OnReceptionActivated") : TEXT("absent"),
					TerminalLabel, false);
				AssertTrue(Record, TEXT("no_security_route"), !bSecurity, TEXT("OnSecurityActivated absent"),
					bSecurity ? TEXT("AdminTerminal OnSecurityActivated") : TEXT("absent"),
					TerminalLabel, false);
				AssertTrue(Record, TEXT("no_records_route"), !bRecords, TEXT("OnRecordsActivated absent"),
					bRecords ? TEXT("AdminTerminal OnRecordsActivated") : TEXT("absent"),
					TerminalLabel, false);
				AssertTrue(Record, TEXT("no_executive_route"), !bExecutive, TEXT("OnExecutiveActivated absent"),
					bExecutive ? TEXT("AdminTerminal OnExecutiveActivated") : TEXT("absent"),
					TerminalLabel, false);
				AssertTrue(Record, TEXT("no_operations_route"), !bOperations, TEXT("OnOperationsActivated absent"),
					bOperations ? TEXT("AdminTerminal OnOperationsActivated") : TEXT("absent"),
					TerminalLabel, false);
			}
			else
			{
				AssertTrue(Record, TEXT("repeat_powered_or_transit"), bScreen || bTransit, TEXT("powered/transit print on second use"),
					FString::Printf(TEXT("screen=%s transit=%s"), *BoolText(bScreen), *BoolText(bTransit)),
					TerminalLabel, false);
			}
			...
			if (Target)
			{
				...
				const FOrganoidPlaytestPropValue HasActivated = OrganoidPlaytestActions::ReadProperty(Target, TEXT("bHasActivated"));
				AssertTrue(
					Record, FString::Printf(TEXT("%s.bHasActivated"), *Prefix),
					HasActivated.bHasBool && HasActivated.bBool,
					TEXT("true"),
					HasActivated.bFound ? HasActivated.Text : TEXT("<missing>"),
					TerminalLabel, false);
			}

			const FNeighborSnapshot ReceptionNow = CaptureNeighbor(World, ReceptionLabel);
			const FNeighborSnapshot SecurityNow = CaptureNeighbor(World, SecurityLabel);
			const FNeighborSnapshot RecordsNow = CaptureNeighbor(World, RecordsLabel);
			const FNeighborSnapshot ExecutiveNow = CaptureNeighbor(World, ExecutiveLabel);
			const FNeighborSnapshot OperationsNow = CaptureNeighbor(World, OperationsLabel);
			AssertTrue(
				Record, FString::Printf(TEXT("%s.reception_unchanged"), *Prefix),
				NeighborUnchanged(ReceptionBefore, ReceptionNow),
				TEXT("unique Reception transform+config unchanged"),
				FString::Printf(TEXT("count=%d id=%s activated=%s"), ReceptionNow.Count, *ReceptionNow.TerminalID, *BoolText(ReceptionNow.bHasActivated)),
				ReceptionLabel, false);
			AssertTrue(
				Record, FString::Printf(TEXT("%s.security_unchanged"), *Prefix),
				NeighborUnchanged(SecurityBefore, SecurityNow),
				TEXT("unique Security transform+config unchanged"),
				FString::Printf(TEXT("count=%d id=%s activated=%s"), SecurityNow.Count, *SecurityNow.TerminalID, *BoolText(SecurityNow.bHasActivated)),
				SecurityLabel, false);
			AssertTrue(
				Record, FString::Printf(TEXT("%s.records_unchanged"), *Prefix),
				NeighborUnchanged(RecordsBefore, RecordsNow),
				TEXT("unique Records transform+config unchanged"),
				FString::Printf(TEXT("count=%d id=%s activated=%s"), RecordsNow.Count, *RecordsNow.TerminalID, *BoolText(RecordsNow.bHasActivated)),
				RecordsLabel, false);
			AssertTrue(
				Record, FString::Printf(TEXT("%s.executive_unchanged"), *Prefix),
				NeighborUnchanged(ExecutiveBefore, ExecutiveNow),
				TEXT("unique Executive transform+config unchanged"),
				FString::Printf(TEXT("count=%d id=%s activated=%s"), ExecutiveNow.Count, *ExecutiveNow.TerminalID, *BoolText(ExecutiveNow.bHasActivated)),
				ExecutiveLabel, false);
			AssertTrue(
				Record, FString::Printf(TEXT("%s.operations_unchanged"), *Prefix),
				NeighborUnchanged(OperationsBefore, OperationsNow),
				TEXT("unique Operations transform+config unchanged"),
				FString::Printf(TEXT("count=%d id=%s activated=%s"), OperationsNow.Count, *OperationsNow.TerminalID, *BoolText(OperationsNow.bHasActivated)),
				OperationsLabel, false);
	...
	struct FS18TransitAutoRegister
	{
		FS18TransitAutoRegister()
		{
			...
			{
				return MakeShared<FS18TransitTerminalFunctional>();
			};
			FOrganoidPlaytestRegistry::Register(Entry);
		}
	};

	static FS18TransitAutoRegister GRegisterS18Transit;
}
