...
namespace
{
	constexpr TCHAR TestId[] = TEXT("S18_OperationsTerminal_Functional");
	constexpr TCHAR DisplayName[] = TEXT("Section 18 Operations Terminal Functional");
	...
	constexpr TCHAR TerminalLabel[] = TEXT("Admin_Terminal_Operations");
	...
	constexpr TCHAR ExpectedTerminalId[] = TEXT("Terminal_AdminOperations");
	constexpr TCHAR ExpectedTitle[] = TEXT("Operations Terminal");
	...
	const FVector ExpectedLocation(4435.0f, 280.0f, 110.0f);
	...
	bool IsOperationsEnum(const FOrganoidPlaytestPropValue& Value)
	{
		if (Value.EnumDisplay.Equals(TEXT("Operations"), ESearchCase::IgnoreCase))
		{
			return true;
		}
		if (Value.EnumInternal.Equals(TEXT("NewEnumerator4"), ESearchCase::IgnoreCase))
		{
			return true;
		}
		if (Value.Text.Equals(TEXT("Operations"), ESearchCase::IgnoreCase))
		{
			return true;
		}
		return Value.bHasNumber && FMath::IsNearlyEqual(Value.Number, 4.0);
	}
	...
	class FS18OperationsTerminalFunctional : public IOrganoidPlaytestCase
	{
	public:
	...
	private:
		...
		void TickWaitPieReady(UProjectOrganoidPlaytestEditorSubsystem& Owner, FOrganoidPlaytestRecord& Record, float DeltaTime)
		{
			...
			if (WaitSeconds > 45.0f)
			{
				...
				Owner.CompleteActive(
					...
					FString::Printf(
						TEXT("Timed out waiting for PIE player + Admin_Terminal_Operations (world=%s pawn=%s terminals=%d walking=%s)."),
		...
		void TickAssertConfig(UProjectOrganoidPlaytestEditorSubsystem& Owner, FOrganoidPlaytestRecord& Record)
		{
			...
			AssertTrue(
				Record,
				TEXT("exactly_one_operations_terminal"),
				Terminals.Num() == 1,
				TEXT("1"),
			...
			if (Terminals.Num() != 1)
			{
				Record.MarkNeedsApproval(
					TEXT("Operations terminal count is not exactly one."),
			...
			AssertTrue(
				...
				TerminalType.bFound && IsOperationsEnum(TerminalType),
				TEXT("Operations / NewEnumerator4 / 4"),
			...
			{
				Record.MarkNeedsApproval(
					TEXT("Durable Operations configuration does not match the expected Section 18 instance."),
					TEXT("Use OrganoidAIBridge prepare_write / dual approve / execute_write. The playtest bot will not mutate."));
				FailAndStop(Owner, Record, Record.FailureReason);
				return;
			}
		...
		void TickWaitFocus(UProjectOrganoidPlaytestEditorSubsystem& Owner, FOrganoidPlaytestRecord& Record, float DeltaTime)
		{
			...
			if (bRange && bFacing && bFocused && bVisitorWest)
			{
				...
				AssertTrue(Record, TEXT("player_visitor_west_side"), true, TEXT("player.X < terminal.X ~ (4335,280)"), FString::SanitizeFloat(Pawn->GetActorLocation().X), TerminalLabel, false);
			...
			if (WaitSeconds > 3.0f)
			{
				...
				AssertTrue(Record, TEXT("player_visitor_west_side"), bVisitorWest, TEXT("player.X < terminal.X ~ (4335,280)"), FString::SanitizeFloat(Pawn->GetActorLocation().X), TerminalLabel, false);
		...
		void TickAssertAfterInteract(UProjectOrganoidPlaytestEditorSubsystem& Owner, FOrganoidPlaytestRecord& Record, float DeltaTime)
		{
			...
			if (InteractAttempt == 1)
			{
				AssertTrue(Record, TEXT("powered_path"), bScreen && !bUnpowered, TEXT("AdminTerminal Screen, no unpowered error"),
					...
					TerminalLabel, false);
				AssertTrue(Record, TEXT("operations_route"), bOperations, TEXT("AdminTerminal OnOperationsActivated"),
					bOperations ? TEXT("AdminTerminal OnOperationsActivated") : TEXT("<missing>"),
					TerminalLabel, false);
				...
				AssertTrue(Record, TEXT("no_executive_route"), !bExecutive, TEXT("OnExecutiveActivated absent"),
					...
					TerminalLabel, false);
				AssertTrue(Record, TEXT("no_transit_route"), !bTransit, TEXT("OnTransitActivated absent"),
					bTransit ? TEXT("AdminTerminal OnTransitActivated") : TEXT("absent"),
					TerminalLabel, false);
			}
			else
			{
				AssertTrue(Record, TEXT("repeat_powered_or_operations"), bScreen || bOperations, TEXT("powered/operations print on second use"),
					FString::Printf(TEXT("screen=%s operations=%s"), *BoolText(bScreen), *BoolText(bOperations)),
					TerminalLabel, false);
			}
	...
	struct FS18OperationsAutoRegister
	{
		FS18OperationsAutoRegister()
		{
			...
			{
				return MakeShared<FS18OperationsTerminalFunctional>();
			};
			FOrganoidPlaytestRegistry::Register(Entry);
		}
	};

	static FS18OperationsAutoRegister GRegisterS18Operations;
}
